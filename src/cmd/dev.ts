...
import { watch_build } from "../build/watch.js";
...
export async function start_dev(server_instance: Hono): Promise<Hono> {
  ...
  console.log(`mode: ${kleur.bold().yellow("dev")}\n`);
  ...
  await watch_build(config, async (build_result) => {
    ...
    await write_pages_html(page_map_internal, hydrationData, "dev");
