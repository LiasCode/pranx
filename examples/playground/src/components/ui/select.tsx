...
export function Select({
  ...
  const [nodeForTheSelectedValue, setNodeForTheSelectedValue] =
    useState<SelectContextT["nodeForTheSelectedValue"]>("");
  ...
  ({ children, className, class: classNative, ...props }) => {
    ...
    useEffect(() => {
      ...
      if (!value) {
        ...
        ref.floating.current
          ?.querySelector<HTMLOptionElement>(`[role=option][data-option-value="${value}"]`)
          ?.focus();
      }
    ...
    const focusKeydownHandle = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowDown") {
        const optionsElements =
          refs.floating.current?.querySelectorAll<HTMLOptionElement>("[role=option]");
