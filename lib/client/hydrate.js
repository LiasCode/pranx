...
const hydratePage = async () => {
  ...
  if (componentModulePath) {
    try {
      ...
      console.error(
        `Prext: Error hydrating component for path ${currentPath} from ${componentModulePath}:`,
        e
      );
    }
  } else {
    console.warn(
      `Prext: No component module path found for client hydration for path: ${currentPath}`
    );
  }
};
